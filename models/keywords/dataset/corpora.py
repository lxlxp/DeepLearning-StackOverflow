...
class STOCorpus(object):
    ...
    def _build_vocab(self, max_vocab_cnt):
        ...
        print("Load corpus with train size %d, test size %d, "
    ...
    def _to_corpus_bow2(self, data, vocab):
        word_cnt = 0
        msg_cnt = 0
        curpus = []
        for post in data:
            new_post = {
                "title": self._sent2id_bow(post["clean_text"].split(), vocab),
                # "content": self._sent2id_bow(post["content"], vocab),
                "category": post["pipe_cate"],
                "id": post["post_id"],
                "raw": post["clean_text"]
            }
            if new_post["title"]:
                curpus.append(new_post)
                word_cnt += len(new_post["title"])
                msg_cnt += 1
        if self.config.tfidf:
            for post in curpus:
                post["title"] = self.tfidf_model[post["title"]]
    ...
    def _read_file(self, path):
        ...
        return train,test,self._process_data(data["non_valid"])
    ...
    def get_corpus(self):
        if self.config.use_dict == "seq":
            ...
            valid_corpus = self._to_corpus_bow2(self.train, vocab)
            non_valid_corpus = self._to_corpus_bow2(self.test, vocab)
