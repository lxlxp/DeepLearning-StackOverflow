...
class KnnDataLoader(object):
    def __init__(self, name, data, vocab, config):
        ...
        if config.use_dict == "seq":
            ...
            for post in data_posts:
                ...
                all_labels.append(post["category"])
    ...
    def _split_shuffle(self, data, all_labels):
        ...
        labels = all_labels
        print(len(data))
        ## extract samples according to distribyutions of classes
        skf = model_selection.StratifiedShuffleSplit(2, test_size=0.4, random_state=0)
        for train_index, test_index in skf.split(data,all_labels):
