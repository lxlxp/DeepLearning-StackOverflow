...
class KeywordBased:
    ...
    def run(self, data_feed, data_test, label_dict, vocab, add_manual=False):
        label_freq_mat = {}
        # new_label_dict = map(label_dict.pop, ["Other", "Architerture:interaction"])
        self.id_label = {y:x for x,y in label_dict.items()}
        for data_ele in data_feed.data:
            # if id_label[data_ele["label"][0]] in ["Other", "Architecture:interaction"]:
            #     continue
            print(data_ele["label"])
            if data_ele["label"] == []:
                continue
            if data_ele["label"] not in label_freq_mat:
                label_freq_mat[data_ele["label"]] = data_ele["text"]
            else:
                label_freq_mat[data_ele["label"]] += data_ele["text"]
        ...
        pred_labels = []
        true_labels = []
        for data_ele in data_test.data:
            # if id_label[data_ele["label"][0]] in ["Other", "Architecture:interaction"]:
            #     continue
            true_labels.append(data_ele["label"])
            pred_label = self._get_vote(data_ele["text"], self.label_vocab_weight)
            # if self.id_label[data_ele["label"]] in ["Debugging:performance"]:
            #     print(self.id_label[pred_label[0]])
            #     # print(data_ele["raw"])
            pred_labels.append(pred_label[0])
        print(accuracy_score(true_labels,pred_labels))
        # self._save_pred(data_test.data, pred_labels, self.id_label, valid=True, true_labels=true_labels)
    ...
    def _my_metric(self, true_labels, pred_labels, label_dict):
        ...
        for i in range(len(true_labels)):
            for j in range(len(pred_labels[i])):
                if pred_labels[i][j] == true_labels[i]:
    ...
    def _save_file(self, label_dict, precisions, recalls):
        id_labels = {y:x for x,y in label_dict.items()}
